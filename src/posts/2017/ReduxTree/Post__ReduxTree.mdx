...
<Note>
  The state tree is a combination of <C>branches</C> and <C>leaves</C>. A branch doesn’t hold any state itself but is a grouping of leaves that each hold chunks of the application state. For example, the branch <C>state.entities</C> groups the states of the <C>posts</C> leaf, <C>comments</C> leaf, etc.
</Note>
